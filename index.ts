...
//
// Flow: read the cached app config (AsyncStorage), apply the brand colour into
// the shared `colors`/`gradients` objects, then require the router. The colour
...
async function loadCachedBrandColor(): Promise<void> {
  try {
...
function Root() {
  ...
  useEffect(() => {
    ...
    void loadCachedBrandColor().finally(() => {
      ...
      const { App } = require('expo-router/build/qualified-entry') as {
        App: ComponentType;
      };
      if (active) {
        setAppComponent(() => App);
